'use client'
// src/app/demo/entregues/page.tsx
// Página de Projetos Entregues no modo demo — dados do contexto
...
export default function DemoEntreguesPage() {
  const { projects, selectedYear } = useDemoContext()

  const entregues = projects
    .filter(p =>
      p.farol === 'azul' &&
      p.data_fim_prevista &&
  ...
  const prevYearProjects = projects.filter(p =>
    p.farol === 'azul' &&
    p.data_fim_prevista &&
